...
const zones = [
  ...
  { zone_id: 'jazz-p3', hotel_id: 'hotelgio', section: 'jazz', floor: 3, circuit: null, label: 'Jazz P3', room_numbers: [], switch_device_id: null, temperature_device_ids: ['jazz-p3-temp'] },
]

test('extracts room and cooling intent from camera report', () => {
...
test('routes Jazz by explicit floor and by Jazz room hundreds digit', () => {
  ...
  assert.equal(selectHvacZone(zones, 'Camera 305 Jazz non fredda').zone.zone_id, 'jazz-p3')
})
...
test('Jazz stays honest when floor switch is not mapped', () => {
  const zone = zones[2]
  const now = Date.parse('2026-08-29T18:40:00Z')
  ...
  const diagnostic = buildHvacDiagnostic({ zone, room: 305, mode: 'cooling', sensors, now })
