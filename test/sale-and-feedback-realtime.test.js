...
test('src/sale-data.js: stesso ponte app<->riga DB già usato per interventi/avvisi urgenti, mappa lo stato a 3 livelli', async () => {
  ...
  assert.match(source, /export async function deleteBookingRow\(id\s*,\s*hotelId\s*=\s*null\)\s*\{/)
  assert.match(source, /const resolved=hotelId\|\|await findCachedHotelId\(ENTITY,id\)/)
