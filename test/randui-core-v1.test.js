...
test('runtime foundation is final and Settings no longer creates a second authenticated chrome', () => {
  ...
  const shellCss = read('../src/randapp/shell.css')
  ...
  assert.doesNotMatch(foundation, /min-width: 1024px\) and \(max-width: 1199px/)
  assert.match(adaptive, /min-width:\s*768px\) and \(max-width:\s*1199px/)
  assert.match(shellCss, /@media \(max-width:\s*1023px\)\s*\{\s*\.rs-sidebar\s*\{\s*display:\s*none;/)
  assert.match(adaptive, /@media \(min-width:\s*1200px\)[\s\S]*\.rs-header\s*\{/s)
  assert.match(shell, /const renderView = \(\) => \{/)
  assert.match(shell, /if \(settings !== null\) return <Settings initialTab=\{settings\}/)
