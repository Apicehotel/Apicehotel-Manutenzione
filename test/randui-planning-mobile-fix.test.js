...
const taskView = read('../src/randapp/operations/TaskView.jsx')
...
test('Operatività and Task do not embed Planning shortcuts', () => {
  ...
  assert.doesNotMatch(taskView, /PlanningCountCards|Scorciatoie planning|Planning operativo|planning-work|planning-sale/)
  ...
  assert.match(taskView, /eyebrow="Task"/)
  assert.match(taskView, /title="Task"/)
  assert.match(taskView, /task-open-urgent/)
  assert.match(taskView, /task-open-reminders/)
  assert.doesNotMatch(taskView, /task-toggle-my-work/)
...
test('Operatività and Task hubs use Planning-style preview cards', () => {
  ...
  assert.match(taskView, /rs-ops-choice-grid|rs-planning-choice-grid/)
  assert.match(taskView, /Avvisi/)
  assert.match(taskView, /Promemoria/)
  assert.doesNotMatch(taskView, /I miei lavori|Interventi|planning-work|planning-sale/)
  assert.doesNotMatch(taskView, /task-toggle-my-work/)
  assert.doesNotMatch(taskView, /taskViewPreviewMetrics/)
})

test('Task hub does not live-reload the whole page on every realtime event', () => {
  assert.match(taskView, /scheduleRefresh|SOFT_REFRESH_MS/)
  assert.doesNotMatch(taskView, /subscribeIssues/)
  assert.doesNotMatch(taskView, /subscribePlanned/)
})
...
test('Operatività and Task hubs paint cards without blocking on full photo hydration', () => {
  ...
  assert.match(taskView, /fetchUrgents/)
  assert.match(taskView, /fetchReminders/)
  assert.doesNotMatch(taskView, /fetchIssues/)
  assert.doesNotMatch(taskView, /fetchPlanned/)
...
test('Operatività and Task flush title above content without stretched dead space', () => {
  ...
  assert.match(visual, /\.rs-task-view\.rs-ops-surface[\s\S]*?align-content:\s*start;/s)
