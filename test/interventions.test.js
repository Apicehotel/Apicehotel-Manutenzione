...
test('Interventi: "Completati" mostra gli interventi completati dentro la stessa scheda (toggle interno), non più redirect a Segnalazioni', async () => {
  const app = await readFile(new URL('../src/App.jsx', import.meta.url), 'utf8')
  assert.match(app, /function InterventionsSection\(\{ items, user, onOpen \}\)/)
  assert.match(app, /const \[search, setSearch\] = useState\(''\), \[view, setView\] = useState\('pending'\)/)
  assert.doesNotMatch(app, /onShowCompleted/)
  assert.match(app, /view === 'done' \? done : pending/)
})
