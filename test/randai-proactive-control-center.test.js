...
test('proactive engine deduplicates repeated global signals inside cooldown', async()=>{
  const store=new ProactiveSignalStore(); const engine=new ProactiveEngine({store,cooldownMs:60_000})
  const a=await engine.ingest({global:true,type:'BUILD_FAILED',fingerprint:'build:main',severity:'HIGH',source:'github'})
  const b=await engine.ingest({global:true,type:'BUILD_FAILED',fingerprint:'build:main',severity:'HIGH',source:'ci'})
...
test('critical proactive signal fails closed instead of acting', async()=>{
  ...
  const signal=await engine.ingest({global:true,type:'PROD_DELETE',fingerprint:'prod-delete',severity:'CRITICAL'})
  const result=await engine.process(signal.id,{global:true,allowAct:true,executeSingle:async()=>({ok:true})})
  assert.equal(result.status,SignalStatus.BLOCKED); assert.equal(called,0)
})

test('high global signal is routed through supervisor and records action', async()=>{
  ...
  const signal=await engine.ingest({global:true,type:'TEST_FAILURE',fingerprint:'test:a',severity:'HIGH'})
  const result=await engine.process(signal.id,{global:true,executeSingle:async()=>({ok:true})})
  assert.equal(result.status,SignalStatus.ACTIONED); assert.equal(result.supervisorRunId,'SUP-1')
})

test('control center projects explicit all-hotel engine state into operational sections', async()=>{
