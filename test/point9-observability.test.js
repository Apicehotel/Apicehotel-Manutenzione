...
const deploymentRecovery = read('src/deployment-recovery.js')
...
test('point 9 completion: stale lazy-module loads recover once without reload loops', () => {
  ...
  assert.match(errorBoundary, /recoverFromDeploymentAssetError/)
  assert.match(deploymentRecovery, /not a valid JavaScript MIME type/i)
  assert.match(deploymentRecovery, /Failed to fetch dynamically imported module/i)
  assert.match(deploymentRecovery, /RECOVERY_WINDOW_MS/)
  assert.match(deploymentRecovery, /recoveryStarted/)
  assert.match(deploymentRecovery, /sessionStorage\.getItem\(key\)/)
  assert.match(deploymentRecovery, /window\.location\.replace\(buildRecoveryUrl\(\)\)/)
  assert.match(deploymentRecovery, /PURGE_RUNTIME_CACHES/)
})
