...
test('Planning Sale: Modifica una prenotazione esistente (prima esisteva solo Elimina, non si poteva correggere sala/date/cliente)', async () => {
  ...
  assert.match(planning, /function SaleBookingForm\(\{ isBusy, onClose, onSave, initial, error \}\) \{/)
...
test("Planning Sale: prenotazioni cliccabili con dettaglio dedicato, Da finire/Fatto visibili anche ai manutentori (non solo a chi può creare/eliminare)", async () => {
  ...
  assert.match(planning, /const markToFinish=async booking=>updateBookingRow\(booking\.id,\{status:'da_finire',toFinishBy:user\.name,toFinishAt:Date\.now\(\)\}\)/)
  assert.match(planning, /const markDone=async booking=>updateBookingRow\(booking\.id,\{status:'done',doneBy:user\.name,doneAt:Date\.now\(\)\}\)/)
  // Nuove prenotazioni partono da 'pending', coerente con Planning lavori, salvate su Supabase.
  assert.match(planning, /await insertBooking\(\{\.\.\.draft,hotelId:hotel\.id,status:'pending',createdBy:user\.name\}\)/)
