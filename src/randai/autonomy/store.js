...
export class SupabaseApprovalStore {
  constructor(client) { if (!client) throw new TypeError('Supabase client is required'); this.client = client }
  async save(item) {
    const row = { id: item.id, identity: item.identity, tool_id: item.toolId, task_id: item.taskId || null, step_id: item.stepId || null, status: item.status, requested_at: item.requestedAt, decided_at: item.decidedAt || null, expires_at: item.expiresAt || null, decided_by: item.decidedBy || null, reason: item.reason || null, payload: item.payload || {} }
  ...
  #map(row) { return { id: row.id, identity: row.identity, toolId: row.tool_id, taskId: row.task_id, stepId: row.step_id, status: row.status, requestedAt: row.requested_at, decidedAt: row.decided_at, expiresAt: row.expires_at, decidedBy: row.decided_by, reason: row.reason, payload: row.payload || {} } }
}
