...
function Login({onReady}){
  const [username,setUsername]=useState('RandAI'),[password,setPassword]=useState('00000000'),[busy,setBusy]=useState(false),[error,setError]=useState('')
  ...
  return <div className="ra-gate"><form className="ra-login" onSubmit={submit}><img src="/icons/randai-cat.webp" alt=""/><small>AREA RISERVATA</small><h1>RandAI Control Center</h1><p>Accedi con le credenziali del ruolo RandAI.</p><label>Username<input autoCapitalize="none" autoCorrect="off" value={username} onChange={(e)=>setUsername(e.target.value)} maxLength={32}/></label><label>Password<input type="password" value={password} onChange={(e)=>setPassword(e.target.value.replace(/[^A-Za-z0-9]/g,'').slice(0,12))} minLength={6} maxLength={12}/><span>6–12 caratteri alfanumerici, senza altri requisiti.</span></label>{error&&<div className="ra-error">{error}</div>}<button className="ra-primary" disabled={busy}>{busy?'Accesso…':'Accedi a RandAI'}</button><button type="button" className="ra-link" onClick={()=>window.location.assign('/')}>← Torna a RandApp</button></form></div>
}
...
function AccessManager({open,onClose,currentUser}){
  ...
  const [name,setName]=useState(''),[username,setUsername]=useState(''),[password,setPassword]=useState('00000000'),[hotels,setHotels]=useState(ALL_HOTELS)
