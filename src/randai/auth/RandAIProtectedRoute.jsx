...
function Login({onReady}){
  ...
  const submit=async(event)=>{event.preventDefault();setError('');if(!isValidRandAIPassword(password)||!isValidRandAIUsername(username)){setError('Username non valido oppure password non alfanumerica da 6 a 12 caratteri.');return}setBusy(true);try{const user=await loginRandAI(username,password);onReady(user)}catch(e){setError(e?.message||'Accesso non riuscito')}finally{setBusy(false)}}
...
export default function RandAIProtectedRoute({mode='control'}){
  ...
  return <><div className={`ra-tools ${live?'ra-tools--live':''}`}><button onClick={()=>setManage(true)}>Accessi RandAI</button><button onClick={logout}>Esci</button></div><Suspense fallback={<div className="ra-gate"><div className="ra-loading">{live?'Caricamento RandAILive…':'Caricamento Control Center…'}</div></div>}>{live?<RandAILive currentUser={state.user}/>:<RandAIControlCenter/>}</Suspense><AccessManager open={manage} onClose={()=>setManage(false)} currentUser={state.user}/></>
}
