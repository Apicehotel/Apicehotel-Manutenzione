...
export default function RepoRadarConsole(){
  ...
  return <section className="rc-ecosystem" aria-label="Repo Radar">
    <div className="rc-kpis">
      <article className="rc-kpi"><span>Candidate</span><strong>{snapshot.candidates.length}</strong><small>stelle solo discovery</small></article>
    ...
    <div className="rc-toolbar"><select value={decision} onChange={(e)=>setDecision(e.target.value)} aria-label="Filtro decisione Repo Radar"><option value="ALL">Tutte le decisioni</option>{ORDER.map((value)=><option key={value} value={value}>{value}</option>)}</select><span className="rc-badge">Weekly scan: GitHub Actions</span><span className="rc-badge good">Auto-install OFF</span></div>
    ...
    <p className="rc-note">Una classificazione ADD/REPLACE/UPGRADE non installa nulla: ogni trapianto resta soggetto a review umana, benchmark, sicurezza, compatibilità e rollback.</p>
  </section>
}
