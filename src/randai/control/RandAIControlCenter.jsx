...
import { HOTELS, TWILIO, WHATSAPP } from '../../config.js'
...
const ADVANCED_NAV = [
  ...
  ['deadlines', 'Scadenze'], ['media', 'Media & Drive'], ['sensors', 'Sensori'],
]
...
export default function RandAIControlCenter() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadHealthy(false)
      setNotice(`Caricamento parziale: ${error?.message || 'errore non disponibile'}`)
  ...
  const systemBar = <div className="rc-system-bar" role="status" aria-label="Stato sistema RandAI"><span><StatusDot state={online ? 'good' : 'bad'} />RandAI {online ? 'Operativo' : 'Offline'}</span><span><StatusDot state="good" />{activeHotelLabel}</span><span><StatusDot state={supabaseState} />Supabase {loadHealthy === false ? 'Errore' : loadHealthy === true ? 'OK' : 'Verifica'}</span><span><StatusDot state={whatsappState} />WhatsApp {whatsappLabel}</span><span><StatusDot state="warn" />Worker Da collegare</span>{lastRefresh && <time>Check {fmt(lastRefresh)}</time>}</div>

  const dashboard = <><div className="rc-kpis"><Card label="Segnalazioni aperte" value={openIssues.length} hint={`${urgentIssues.length} ad alta priorità`} tone={urgentIssues.length ? 'danger' : ''} /><Card label="Manutenzioni attive" value={upcoming.length} hint="programmate / da completare" /><Card label="Conoscenze" value={scopedProcedures.length} hint={`${scopedProcedures.filter((p) => p.status === 'approved').length} approvate`} /><Card label="Impianti" value={scopedEquipment.filter((e) => e.active !== false).length} hint={`${scopedDocs.length} fonti collegate`} /></div><div className="rc-dashboard-grid"><Panel title="Stato sistema" meta={loadHealthy === false ? 'attenzione' : 'fondamenta attive'}><div className="rc-health-list"><div><span><StatusDot state={online ? 'good' : 'bad'} />Connessione browser</span><Badge tone={online ? 'good' : 'bad'}>{online ? 'online' : 'offline'}</Badge></div><div><span><StatusDot state={supabaseState} />Supabase / dati RandApp</span><Badge tone={supabaseState}>{loadHealthy === true ? 'ok' : loadHealthy === false ? 'errore' : 'verifica'}</Badge></div><div><span><StatusDot state={whatsappState} />WhatsApp / Twilio</span><Badge tone="warn">{whatsappLabel}</Badge></div><div><span><StatusDot state="warn" />Registro worker</span><Badge tone="warn">step 2+</Badge></div></div></Panel><Panel title="Permessi console" meta={`${access.hotels.length} strutture`}><div className="rc-permission-card"><strong>{access.name}</strong><p>Accesso amministrativo RandAI verificato tramite membership attiva e <code>can_access_admin</code>.</p><div className="rc-chip-row">{access.hotels.map((id) => <Badge key={id} tone="good">{HOTEL_LABELS[id] || id}</Badge>)}</div></div></Panel><Panel title="Priorità operative" meta={`${urgentIssues.length} critiche`}>{urgentIssues.length ? <div className="rc-stack">{urgentIssues.slice(0, 7).map((i) => <button key={i.id} className="rc-row-button" onClick={() => setSelectedIssue(i)}><span><strong>{i.title || i.category}</strong><small>{HOTEL_LABELS[i.hotelId]} · {i.room || '—'}</small></span><Badge tone="bad">{i.urgency}</Badge></button>)}</div> : <Empty>Nessuna priorità alta aperta.</Empty>}</Panel><Panel title="Prossime scadenze" meta={`${upcoming.length} attive`}><div className="rc-stack">{upcoming.slice(0, 7).map((i) => <div className="rc-row" key={i.id}><span><strong>{i.category || i.notes || 'Intervento'}</strong><small>{HOTEL_LABELS[i.hotelId]} · {i.location || '—'}</small></span><time>{fmt(i.scheduledAt)}</time></div>)}{!upcoming.length && <Empty />}</div></Panel></div></>

  const whatsappPanel = <div className="rc-dashboard-grid"><Panel title="Canali WhatsApp" meta={whatsappConfigured ? 'integrazione configurata' : 'fondazione pronta'}><div className="rc-stack">{HOTELS.filter((hotel) => access.hotels.includes(hotel.id)).map((hotel) => { const destination = WHATSAPP?.destinations?.[hotel.id]; const configured = Boolean(destination?.phone); return <div className="rc-channel" key={hotel.id}><span><strong>{hotel.name}</strong><small>{configured ? `Destinazione ${destination.phone}` : 'Nessun numero configurato nel codice'}</small></span><Badge tone={configured ? 'good' : 'warn'}>{configured ? 'configurato' : 'da collegare'}</Badge></div> })}</div></Panel><Panel title="Ingresso Twilio" meta="stato reale da configurazione"><div className="rc-integration-state"><Badge tone={whatsappConfigured ? 'good' : 'warn'}>{whatsappLabel}</Badge><h3>{whatsappConfigured ? 'Webhook configurato' : 'Webhook inbound non configurato'}</h3><p>La console non mostra falsi messaggi o stati online: l’Inbox sarà alimentata soltanto quando il webhook reale sarà presente e verificato.</p></div></Panel><Panel title="Inbox" meta="Step 2"><Empty>Nessun feed WhatsApp collegato. La shell è pronta per messaggi, foto, stato elaborazione, pausa e retry.</Empty></Panel><Panel title="Sicurezza ingresso" meta="deny by default"><div className="rc-permission-card"><strong>Nessuna creazione automatica simulata</strong><p>Finché l’ingresso non è collegato e verificato, RandAI non trasforma messaggi WhatsApp in segnalazioni.</p></div></Panel></div>

  const workersPanel = <div className="rc-dashboard-grid"><Panel title="Registro Worker" meta="0 registrati"><Empty>Nessun worker è dichiarato online senza una sorgente di stato verificabile. Il registro è pronto per nome, frequenza, ultima esecuzione, prossima esecuzione ed esito.</Empty></Panel><Panel title="Health worker" meta="fondazione"><div className="rc-health-list"><div><span><StatusDot state="warn" />Scheduler</span><Badge tone="warn">da collegare</Badge></div><div><span><StatusDot state="warn" />Ultima esecuzione</span><Badge>—</Badge></div><div><span><StatusDot state="warn" />Errori / retry</span><Badge>—</Badge></div></div></Panel></div>
