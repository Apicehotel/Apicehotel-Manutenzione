...
import { ThemeControl } from '../../randapp/ui.jsx'
...
import AgentControlPanel from './AgentControlPanel.jsx'
import { buildAgentRuntimeBoard } from '../control-center/agent-registry.js'
import './randai-control.css'
import './ecosystem-control.css'
import './randai-responsive.css'

const PRIMARY_NAV = [['dashboard', 'Overview'], ['agents', 'Agenti IA'], ['capabilities', 'Funzioni'], ['configuration', 'Configurazione'], ['ecosystem', 'Salute sistema']]
...
export default function RandAIControlCenter() {
  ...
  const [agentRuntimeRows, setAgentRuntimeRows] = useState([])
  ...
  useEffect(() => {
    if (!supabase || !access.allowed) return undefined
    let active = true
    const refreshRuntime = async () => {
      const { data, error } = await supabase.from('randcore_agent_runtime').select('agent_id,status,desired_state,heartbeat_at,task_id,activity,detail,hotel_id,updated_at').order('agent_id')
      if (!active || error) return
      setAgentRuntimeRows(data || [])
      setRuntimeNow(Date.now())
    }
    refreshRuntime()
    const tick = window.setInterval(() => setRuntimeNow(Date.now()), 30_000)
    const channel = supabase.channel('randai-overview-live-runtime').on('postgres_changes', { event: '*', schema: 'public', table: 'randcore_agent_runtime' }, refreshRuntime).subscribe()
    return () => { active = false; window.clearInterval(tick); supabase.removeChannel(channel) }
  }, [access.allowed])
  ...
  const liveAgentBoard = useMemo(() => buildAgentRuntimeBoard(agentRuntimeRows.map((row) => ({ ...row, agentId: row.agent_id, heartbeatAt: row.heartbeat_at, taskId: row.task_id, hotelId: row.hotel_id, updatedAt: row.updated_at })), runtimeNow), [agentRuntimeRows, runtimeNow])
  const liveAgentCounts = useMemo(() => liveAgentBoard.reduce((acc, agent) => { acc[agent.status] = (acc[agent.status] || 0) + 1; return acc }, {}), [liveAgentBoard])
  const latestAgentActivity = useMemo(() => [...liveAgentBoard].filter((agent) => agent.heartbeatAt).sort((a, b) => new Date(b.heartbeatAt).getTime() - new Date(a.heartbeatAt).getTime()).slice(0, 6), [liveAgentBoard])
