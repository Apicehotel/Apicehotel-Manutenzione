...
import { evaluateMonthlyHealthCadence, healthCadenceLabel } from '../core/health-cadence.js'
...
export default function RandCoreHealthConsole({accessHotels=[],hotelFilter='all'}){
  ...
  const cadence=useMemo(()=>evaluateMonthlyHealthCadence(latest),[latest?.id,latest?.created_at,latest?.evaluated_at])
  ...
  return <div className="rc-health-console">
    <div className="rc-kpis">
      ...
      <article className="rc-kpi"><span>Check mensile</span><strong className={`rc-badge ${cadence.state==='FRESH'?'good':cadence.state==='MISSING'||cadence.state==='OVERDUE'?'bad':'warn'}`}>{cadence.state}</strong><small>{healthCadenceLabel(cadence)}</small></article>
    </div>
    ...
    {cadence.state!=='FRESH'&&<div className="rc-notice">Check mensile: {healthCadenceLabel(cadence)}. Il dato resta informativo e non sostituisce l’autorizzazione RandSecure.</div>}
  </div>
}
