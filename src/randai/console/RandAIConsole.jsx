...
export default function RandAIConsole(){
  ...
  const edit=async(r)=>{
    setSelectedId(r.id);setTestResult(null);setNotice('')
    const base={...EMPTY,hotel_id:r.hotel_id,title:r.title||'',category:r.category||'',area:r.area||'',symptom:r.symptom||'',summary:r.summary||'',steps_text:(r.steps||[]).join('\n'),caution:r.caution||'',source_label:r.source_label||'Conoscenza interna RandAI',status:r.status||'draft'}
    setForm(base)
    const {data:doc}=await supabase.from('randai_documents').select('equipment_id,source_type,source_label,external_url,media_kind').eq('procedure_id',r.id).order('updated_at',{ascending:false}).limit(1).maybeSingle()
    if(doc) setForm({...base,equipment_id:doc.equipment_id||'',source_type:doc.source_type||'procedura_interna',source_label:doc.source_label||base.source_label,drive_url:doc.external_url||'',media_kind:doc.media_kind||'document'})
  }
  ...
  const removeMedia=async()=>{if(!selectedId)return;setBusy(true);const {error}=await supabase.from('randai_documents').delete().eq('procedure_id',selectedId);setBusy(false);if(error)setNotice(`Rimozione allegato non riuscita: ${error.message}`);else{setForm((f)=>({...f,drive_url:'',media_kind:'document'}));setNotice('Allegato rimosso.')}}
