...
export class RandDurableRuntime {
  ...
  async start({ workflow, input = {}, context = {}, idempotencyKey, maxAttempts = 3 } = {}) {
    ...
    if (!auth?.allowed) return freeze({ status: 'DENIED', authorization: auth })
    const now = this.clock()
    const run = {
      id: `run_${now}_${Math.random().toString(36).slice(2, 10)}`,
      workflowId: clean(workflow.id), workflowVersion: clean(workflow.version || '1'),
      hotelId: auth.hotelId, actorId: auth.actorId, input: structuredClone(input),
      checkpoint: null, attempt: 0, maxAttempts: Math.max(1, Number(maxAttempts) || 3),
      status: DurableStatus.PENDING, idempotencyKey: key, createdAt: now, updatedAt: now,
    }
    await this.store.put(run); await this.store.bindKey(key, run.id)
  ...
  async resume({ runId, workflow, context = {} } = {}) {
    ...
    run.status = DurableStatus.RUNNING; run.attempt += 1; run.updatedAt = this.clock(); await this.store.put(run)
    return traceRandAIOperation('durable.resume', { 'rand.hotel_id': run.hotelId, 'rand.workflow_id': run.workflowId, 'rand.workflow_attempt': run.attempt }, async () => {
      try {
        const result = await workflow.execute({ input: structuredClone(run.input), checkpoint: structuredClone(run.checkpoint), knowledge, run: freeze({ ...run }) })
        if (result?.wait === true) {
  ...
  async #finish(run, status, extra = {}) {
    ...
    return freeze(structuredClone(run))
  }
}
