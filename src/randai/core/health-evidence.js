...
export const EvidenceFreshness = Object.freeze({
  FRESH: 'FRESH',
  STALE: 'STALE',
  UNKNOWN: 'UNKNOWN',
})
...
export function normalizeDomainEvidence(domain, input = {}, {
  ...
  const status = HEALTH_STATUSES.includes(String(input.status || '').toUpperCase())
  ...
  const maxAgeMs = Number.isFinite(Number(input.maxAgeMs)) && Number(input.maxAgeMs) > 0
  ...
  const freshness = !checkedMs
    ? EvidenceFreshness.UNKNOWN
    : nowMs - checkedMs > maxAgeMs
...
export function buildHealthEvidenceSnapshot({ domains = {}, generatedAt = new Date().toISOString() } = {}) {
  const normalized = Object.fromEntries(RANDCORE_HEALTH_DOMAINS.map((domain) => [
    domain,
    normalizeDomainEvidence(domain, domains[domain] || {}, { now: generatedAt }),
  ]))
  ...
  const verifiedCoverage = Math.round((verified.length / RANDCORE_HEALTH_DOMAINS.length) * 100)
  const confidence = Math.round((values.reduce((sum, item) => sum + item.confidence, 0) / RANDCORE_HEALTH_DOMAINS.length))
  const status = values.some((item) => item.status === 'CRITICAL')
    ? 'CRITICAL'
    : values.some((item) => item.status === 'DEGRADED') || stale.length > 0 || unknown.length > 0
      ? 'DEGRADED'
      : verified.length === RANDCORE_HEALTH_DOMAINS.length
        ? 'HEALTHY'
        : 'UNKNOWN'

  return Object.freeze({
    version: RANDCORE_HEALTH_CONTRACT_VERSION,
    ...
    coverage: Object.freeze({
      ...
      verified_percent: verifiedCoverage,
    }),
...
export function fromLegacyHealthSnapshot(snapshot = {}, { generatedAt } = {}) {
  ...
  const domains = Object.fromEntries(RANDCORE_HEALTH_DOMAINS.map((domain) => {
    ...
    return [domain, {
      status: measured ? (legacy.status || 'HEALTHY') : 'UNKNOWN',
      score: measured ? (legacy.score ?? 100) : null,
      checkedAt: legacy.checkedAt || legacy.checked_at || checkedAt,
      measured,
      source: measured ? (legacy.source || 'legacy-randcore') : null,
      evidence: measured ? legacy : null,
      details: legacy,
      confidence: measured ? 100 : 0,
    }]
  }))
