...
const ACTIVE='active', SUPERSEDED='superseded', FORGOTTEN='forgotten', LEGAL_HOLD='legal_hold'
...
export function usableAt(memory,asOf){
  ...
  const forgottenAt=dateMs(memory.forgottenAt); if(memory.lifecycleStatus===FORGOTTEN && (!Number.isFinite(forgottenAt)||forgottenAt<=at)) return false
  const supersededAt=dateMs(memory.supersededAt); if(memory.lifecycleStatus===SUPERSEDED && (!Number.isFinite(supersededAt)||supersededAt<=at)) return false
...
export function suggestConflictWinner(items=[],now=Date.now(),qualityFn){
  if(typeof qualityFn!=='function') throw new TypeError('qualityFn is required for conflict ranking')
  const ranked=items.map((memory)=>({memory,quality:qualityFn(memory,now)})).sort((a,b)=>b.quality.score-a.quality.score || String(a.memory.id).localeCompare(String(b.memory.id)))
...
export function planRetention(items=[],policy={},now=Date.now()){
  ...
  for(const memory of items){
    if(memory.lifecycleStatus!==ACTIVE || memory.retentionClass===LEGAL_HOLD) continue
    ...
    if(dueAt<=now) candidates.push({id:memory.id,retentionClass:memory.retentionClass,dueAt:new Date(dueAt).toISOString(),reason:`RETENTION_${String(memory.retentionClass).toUpperCase()}_EXPIRED`})
  }
