...
export function evaluateRepoCandidate(candidate,{incumbent=null,minAddScore=.78,minReplaceDelta=.08}={}){
  ...
  const total=Object.entries(WEIGHTS).reduce((sum,[key,weight])=>sum+evidence[key]*weight,0)
  ...
  if(blockers.length){ decision=RepoRadarDecision.REJECT; reason=blockers[0] }
    ...
    else { superiorityDelta=total-score(incumbent.score); if(allPass&&superiorityDelta>=minReplaceDelta){ decision=RepoRadarDecision.REPLACE; reason='MEASURABLY_SUPERIOR_WITH_SAFE_ROLLBACK' } else reason=hasUnknown?'REPLACEMENT_GATES_INCOMPLETE':'SUPERIORITY_NOT_DEMONSTRATED' }
  ...
  return Object.freeze({ id:candidate.id,name:candidate.name,repository:candidate.repository,decision,reason,score:Number(total.toFixed(4)),superiorityDelta:superiorityDelta==null?null:Number(superiorityDelta.toFixed(4)),gates,blockers,evidence,stars:Number(candidate.stars||0),note:candidate.note||'',evaluatedAt:candidate.evaluatedAt||null })
}
