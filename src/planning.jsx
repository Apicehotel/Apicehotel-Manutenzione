...
function SaleBookingForm({ isBusy, onClose, onSave, initial }) {
...
export function PlanningSale({ user, openRequest }) {
  ...
  const save=draft=>{if(editingBookingId){if(isBusy(draft.room,draft.dateFrom,draft.dateTo,draft.shift,editingBookingId))return;persist(bookings.map(item=>item.id===editingBookingId?{...item,...draft}:item));setEditingBookingId(null)}else{if(isBusy(draft.room,draft.dateFrom,draft.dateTo,draft.shift))return;persist([{...draft,id:Date.now(),status:'pending',createdBy:user.name,createdAt:Date.now()},...bookings])}setCreating(false)}
