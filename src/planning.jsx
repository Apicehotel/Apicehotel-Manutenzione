...
export function PlanningSale({ user, openRequest }) {
  ...
  const save=draft=>{if(isBusy(draft.room,draft.dateFrom,draft.dateTo,draft.shift))return;persist([{...draft,id:Date.now(),status:'pending',createdBy:user.name,createdAt:Date.now()},...bookings]);setCreating(false)}
  ...
  const markToFinish=booking=>persist(bookings.map(item=>item.id===booking.id?{...item,status:'da_finire',toFinishBy:user.name,toFinishAt:Date.now()}:item))
  ...
  return <section className="calendar-page sale-calendar"><header><div><h2>Planning Sale</h2><p>Prenotazioni sale sempre definite con periodo Da/A.</p></div>{!canEdit&&<span>Sola visualizzazione</span>}</header><CalendarControls view={view} onView={setView} anchor={anchor} onAnchor={setAnchor}/><div className="shift-legend">{Object.entries(SHIFT_LABELS).map(([key,label])=><span key={key} className={key}><i/>{label}</span>)}</div><div className="calendar-days">{days.map(day=>{const date=isoDay(day);const entries=bookings.filter(item=>(item.dateFrom||item.date)<=date&&(item.dateTo||item.date)>=date).sort((a,b)=>SALE_DEF.findIndex(([room])=>room===a.room)-SALE_DEF.findIndex(([room])=>room===b.room));return <article key={date}><header><strong>{dayLabel(day)}</strong>{date===isoDay(new Date())&&<span>OGGI</span>}</header>{entries.length?<div>{entries.map(item=><div className={`sale-event ${item.shift} ${item.status||'pending'}`} key={item.id}><div><strong>{item.room}</strong><span>{item.client}{item.notes?` · ${item.notes}`:''}</span>{item.status==='da_finire'&&<small className="sale-status-note">Da finire · {item.toFinishBy}</small>}{item.status==='done'&&<small className="sale-status-note">✓ Fatto · {item.doneBy}</small>}</div><small>{SHIFT_LABELS[item.shift]} · {item.dateFrom||item.date} → {item.dateTo||item.date}</small>{canEdit&&<div className="sale-event-actions">{item.status!=='done'&&<button className="sale-status-btn" onClick={()=>markToFinish(item)}>◐ Da finire</button>}{item.status!=='done'&&<button className="sale-status-btn done" onClick={()=>markDone(item)}>✓ Fatto</button>}<button onClick={()=>remove(item)} aria-label={`Elimina ${item.client}`}>×</button></div>}</div>)}</div>:<p>Nessuna prenotazione</p>}</article>})}</div>{creating&&<SaleBookingForm isBusy={isBusy} onClose={()=>setCreating(false)} onSave={save}/>}</section>
}
