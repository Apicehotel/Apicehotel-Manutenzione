...
function SaleBookingForm({ isBusy, onClose, onSave, initial, error }) {
  ...
  const occupied=draft.room&&validRange&&isBusy(draft.room,draft.dateFrom,draft.dateTo,draft.shift,initial?.id)
...
export function PlanningSale({ hotel, user, openRequest }) {
  ...
  const save=async draft=>{
    ...
    try {
      if(editingBookingId){
        if(isBusy(draft.room,draft.dateFrom,draft.dateTo,draft.shift,editingBookingId))return
        await updateBookingRow(editingBookingId,draft)
        setEditingBookingId(null)
      } else {
        if(isBusy(draft.room,draft.dateFrom,draft.dateTo,draft.shift))return
        await insertBooking({...draft,hotelId:hotel.id,status:'pending',createdBy:user.name})
      }
      setCreating(false)
    } catch(error) {
      setSaveError(error?.message||'Salvataggio non riuscito, riprova')
    }
  }
  ...
  const markToFinish=async booking=>updateBookingRow(booking.id,{status:'da_finire',toFinishBy:user.name,toFinishAt:Date.now()})
  const markDone=async booking=>updateBookingRow(booking.id,{status:'done',doneBy:user.name,doneAt:Date.now()})
