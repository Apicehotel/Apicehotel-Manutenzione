...
function normalizeDisposition(confidenceDecision = {}) {
  const value = confidenceDecision.disposition || confidenceDecision.decision
  return Object.prototype.hasOwnProperty.call(ORDER, value) ? value : null
}
...
export function decideExecutionPolicy({ hotelId, planValidation, confidenceDecision, permissionGranted = false, approvalPresent = false } = {}) {
  ...
  const decision = normalizeDisposition(confidenceDecision)
  if (!decision) return { decision: 'BLOCK', reason: 'CONFIDENCE_DECISION_MISSING' }
