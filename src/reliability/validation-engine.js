...
function issue(path, code, message, meta = null) {
...
export function required(value, path, message = `${path} è obbligatorio`) {
  return text(value) ? [] : [issue(path, ValidationCode.REQUIRED, message)]
}

export function oneOf(value, allowed, path, { optional = false } = {}) {
  if ((value === undefined || value === null || value === '') && optional) return []
  return allowed.includes(value) ? [] : [issue(path, ValidationCode.INVALID_VALUE, `${path} non valido`, { allowed })]
}

export function maxLength(value, max, path, { optional = true } = {}) {
  if ((value === undefined || value === null || value === '') && optional) return []
  return text(value).length <= max ? [] : [issue(path, ValidationCode.TOO_LONG, `${path} supera ${max} caratteri`, { max })]
}
...
export function finiteNumber(value, path, { min = null, max = null, optional = false, integer = false, nonZero = false } = {}) {
  ...
  if (!Number.isFinite(number) || (integer && !Number.isInteger(number)) || (nonZero && number === 0)) {
    return [issue(path, ValidationCode.INVALID_NUMBER, `${path} deve essere un numero valido`)]
  }
  if ((min !== null && number < min) || (max !== null && number > max)) {
    return [issue(path, ValidationCode.OUT_OF_RANGE, `${path} fuori intervallo`, { min, max })]
  }
  return []
}
...
export function isoDate(value, path, { optional = false } = {}) {
  ...
  return Number.isFinite(parsed) ? [] : [issue(path, ValidationCode.INVALID_DATE, `${path} non è una data valida`)]
}
...
export function dateRange(from, to, fromPath = 'dateFrom', toPath = 'dateTo') {
  ...
  return Date.parse(to) >= Date.parse(from)
    ? []
    : [issue(toPath, ValidationCode.INVALID_DATE_RANGE, `${toPath} non può precedere ${fromPath}`)]
}
...
export function transition(current, next, transitions, path = 'status', { allowSame = true } = {}) {
  ...
  return allowed.includes(next)
    ? []
    : [issue(path, ValidationCode.INVALID_STATE_TRANSITION, `Transizione ${current} → ${next} non consentita`, { current, next, allowed })]
}
