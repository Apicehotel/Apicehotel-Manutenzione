...
function TransformUrgentForm({urgent,hotel,user,onCancel,onDone}){
  const[location,setLocation]=useState(''),[category,setCategory]=useState(ISSUE_CATEGORIES[0]),[urgency,setUrgency]=useState('alta'),[note,setNote]=useState(urgent.note||''),[saving,setSaving]=useState(false)
  const submit=async e=>{e.preventDefault();if(!location.trim()||!note.trim()||saving)return;setSaving(true);try{const issue=await insertIssue({hotelId:hotel.id,room:location.trim(),category,urgency,title:note.trim(),status:'todo',createdAt:Date.now(),createdByName:user?.name||'App',origin:'App'});await linkUrgentToIssue(urgent.id,hotel.id,issue.id,user?.name);onDone()}finally{setSaving(false)}}
  return <form className="rs-form" onSubmit={submit} data-testid="transform-urgent-form"><div className="rs-form__head"><IconButton icon="chevronLeft" label="Indietro" onClick={onCancel}/><div><h2>Trasforma in segnalazione</h2><p>{hotel.name}</p></div></div><div className="rs-note rs-note--waiting">Richiesta urgente originale, da <strong>{urgent.createdBy}</strong>: “{urgent.note}”<br/>Verrà segnata come gestita e collegata alla nuova segnalazione.</div><Field label="Camera o zona"><TextInput value={location} onChange={e=>setLocation(e.target.value)} placeholder="Es. Camera 214 o Hall" required autoFocus/></Field><fieldset className="rs-fieldset"><legend>Categoria</legend><div className="rs-chips">{ISSUE_CATEGORIES.map(item=><button type="button" key={item} className={`rs-chip ${category===item?'active':''}`} onClick={()=>setCategory(item)}>{item}</button>)}</div></fieldset><fieldset className="rs-fieldset"><legend>Urgenza</legend><div className="rs-chips">{Object.entries(URGENCY_META).map(([k,v])=><button type="button" key={k} className={`rs-chip ${urgency===k?'active':''}`} onClick={()=>setUrgency(k)}>{v.label}</button>)}</div></fieldset><Field label="Note"><textarea className="rs-textarea" rows="4" value={note} onChange={e=>setNote(e.target.value)} required/></Field><div className="rs-form-actions"><Button type="button" variant="ghost" onClick={onCancel}>Annulla</Button><Button variant="primary" icon="plus" disabled={!location.trim()||!note.trim()||saving}>{saving?'Creo…':'Crea segnalazione e chiudi urgenza'}</Button></div></form>
}
