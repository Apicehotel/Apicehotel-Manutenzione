...
export function ManualView({user,hotel}){
  const[procedures,setProcedures]=useState([]),[query,setQuery]=useState(''),[message,setMessage]=useState('')
  const allowed=Boolean(hotel?.id&&canUser(user,'procedures','view'))
  useEffect(()=>{let active=true;if(!allowed||!supabase)return undefined;supabase.from('randai_procedures').select('id,title,category,area,summary,steps,caution,source_label,version').eq('hotel_id',hotel.id).eq('status','approved').order('title').then(({data,error})=>{if(!active)return;if(error)setMessage('RandGuide non disponibile in questo momento.');else setProcedures(data||[])});return()=>{active=false}},[allowed,hotel?.id])
  const needle=query.trim().toLowerCase(),visible=procedures.filter((item)=>!needle||`${item.title} ${item.category||''} ${item.area||''} ${item.summary||''}`.toLowerCase().includes(needle))
  const quick=[['Segnalazioni','Il pulsante + è contestuale: in Segnalazioni crea direttamente una nuova segnalazione.'],['Interventi','Consulta i lavori assegnati e aggiorna lo stato quando il lavoro è completato.'],['Avvisi urgenti','Gli avvisi urgenti sono sincronizzati in tempo reale e possono essere presi in carico.'],['Planning','Nel Planning il + propone solo le nuove attività compatibili con i tuoi permessi.'],['Rubrica tecnici','In Rubrica tecnici il + apre direttamente Nuovo tecnico e salva nell’anagrafica tecnici esterni.'],['Struttura','Tocca il nome dell’hotel nell’intestazione per cambiare struttura quando il tuo account ne gestisce più di una.'],['Aspetto','Tema Sistema/Chiaro/Scuro e dimensione Piccolo/Normale/Grande sono disponibili in Profilo e nel menu.']]
  return <div data-testid="manual-view"><PageTitle title="Manuale e RandGuide" subtitle={hotel?.name||'Guida operativa RandApp'}/>{allowed&&<Card className="rs-card--pad"><Field label="Cerca nelle procedure approvate"><TextInput icon="search" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Impianto, zona, problema…"/></Field>{message&&<p className="rs-error">{message}</p>}<div className="rs-migrated-list">{visible.map((item)=><details className="rs-card rs-card--pad rs-manual-card" key={item.id}><summary><strong>{item.title}</strong><small>{item.category||'Procedura'}{item.area?` · ${item.area}`:''} · v{item.version}</small></summary><p>{item.summary}</p>{Array.isArray(item.steps)&&item.steps.length>0&&<ol>{item.steps.map((step,index)=><li key={`${item.id}-${index}`}>{typeof step==='string'?step:step?.title||step?.text||String(step)}</li>)}</ol>}{item.caution&&<p><strong>Attenzione:</strong> {item.caution}</p>}<small>Fonte: {item.source_label||'Procedura interna approvata'}</small></details>)}{!message&&!visible.length&&<EmptyState icon="search" title="Nessuna procedura">Non risultano procedure approvate corrispondenti.</EmptyState>}</div></Card>}<div className="rs-migrated-list">{quick.map(([title,text])=><Card key={title} className="rs-card--pad rs-manual-card"><strong>{title}</strong><p>{text}</p></Card>)}</div></div>
}
