...
const isPaolo = (user) => user?.auth_user_id === PAOLO_AUTH_USER_ID || String(user?.name || '').trim().toLowerCase() === 'paolo'

export default function InsertLauncher({ open, onClose, hotel, user, onPick }) {
  const visibleActions = ACTIONS.filter((item) => item.id !== 'planning-sale' || isPaolo(user))
  const pick = (id) => {
    if (id === 'intervention' || id === 'planning-work') {
      try { sessionStorage.setItem('randapp.insert-source', id) } catch { /* continua anche senza storage */ }
    }
    onPick(id)
  }
  return <Sheet open={open} onClose={onClose} className="rs-insert-shell">
    ...
    <p style={{margin:'14px 2px 0',color:'var(--rs-text-3)',fontSize:'.72rem',lineHeight:1.4}}>{isPaolo(user) ? 'Ogni voce apre direttamente il relativo inserimento.' : 'Planning sale è gestito da Paolo; qui restano disponibili gli inserimenti operativi.'}</p>
  </Sheet>
}
