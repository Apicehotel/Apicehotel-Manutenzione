...
export default function SensorsTab(){
  ...
  if(loading)return <Spinner label="Carico sensori e impianti…"/>
  return <section data-testid="settings-sensors"><div className="rs-page-title"><div><h1>Sensori e impianti</h1><p>Visibilità dei dispositivi eWeLink per struttura. La sincronizzazione avviene automaticamente sul server.</p></div><Button variant="ghost" icon="refresh" onClick={refresh} disabled={refreshing}>{refreshing?'Ricarico…':'Ricarica'}</Button></div>{sensors.length===0?<EmptyState icon="sensor" title="Nessun dispositivo">Nessun dato eWeLink ancora disponibile.</EmptyState>:sensors.map(s=><Card key={s.device_id} className="rs-sensor"><div className="rs-sensor__info"><strong>{s.nome||s.device_id}</strong><small>{s.temperatura!=null?`${s.temperatura}°C`:!s.online?'NON DISPONIBILE':s.switch_state==='on'?'ATTIVO':s.switch_state==='off'?'SPENTO':'Stato non disponibile'}</small></div><div className="rs-hotel-toggles">{HOTELS.map(h=><button key={h.id} className={`rs-hotel-toggle ${s[`mostra_${h.id}`]?'on':''}`} onClick={()=>toggle(s,h.id)}>{s[`mostra_${h.id}`]?'✓ ':''}{h.short}</button>)}</div></Card>)}</section>
}
