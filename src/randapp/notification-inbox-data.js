...
export async function fetchNotificationInbox(hotelId, user) {
  ...
  const items = [...urgentItems, ...reminderItems.filter(Boolean)].sort((a, b) => new Date(b.at || 0) - new Date(a.at || 0)).slice(0, 100)
...
export async function markNotificationRead(hotelId, item) {
  ...
  if (!userId) return
  const { error } = await supabase.from('notification_reads').upsert({ hotel_id: hotelId, source_type: item.type, source_id: String(item.sourceId), user_id: userId, read_at: new Date().toISOString() }, { onConflict: 'user_id,hotel_id,source_type,source_id' })
  if (error) throw error
}
...
export async function markAllNotificationsRead(hotelId, items) {
  ...
  const rows = items.filter(x => !x.read).map(item => ({ hotel_id: hotelId, source_type: item.type, source_id: String(item.sourceId), user_id: userId, read_at: now }))
