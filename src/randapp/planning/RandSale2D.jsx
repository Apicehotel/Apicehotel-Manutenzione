...
export default function RandSale2D({ booking, user, onClose }) {
  ...
  const save=async()=>{setBusy(true);setError('');try{const saved=await saveSaleLayoutSnapshot({bookingId:booking.id,hotelId:booking.hotelId,document:bindLayoutDocument(document,booking),expectedVersion:snapshot?.version||0});setSnapshot(saved);setHistory([bindLayoutDocument(saved.document,booking)]);setCursor(0)}catch(err){setError(err.message||'Pianta non salvata')}finally{setBusy(false)}}
