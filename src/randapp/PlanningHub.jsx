...
import PlanningSaleSimple, { canOperateSalePlanning } from './PlanningSaleSimple.jsx'
...
export default function PlanningHub({hotel,user,createRequest=null,allowSale=true}){
  ...
  const load=useCallback(async()=>{setLoading(true);try{const [workItems,sales]=await Promise.all([fetchPlanningWork(hotel.id),canSeeSale?fetchBookings(hotel.id):Promise.resolve({items:[]})]);setWork(workItems||[]);setBookings(sales.items||[])}finally{setLoading(false)}},[hotel.id,canSeeSale])
