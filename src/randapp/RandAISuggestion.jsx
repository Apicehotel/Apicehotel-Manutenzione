...
export default function RandAISuggestion({ issue, hotelId, user = null, onActionExecuted = null, onCompletionSummary = null }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (cancelled) return
      if (guidanceResult.status === 'fulfilled') setGuidanceState({ loading: false, guidance: guidanceResult.value, unavailable: false })
      else setGuidanceState({ loading: false, guidance: null, unavailable: true })
  ...
  const confirmStep = async () => {
    ...
    try {
      ...
      setStepNote('')
      setWorkspaceState({ loading: false, task, busy: false, error: '', success: task?.status === 'VERIFYING' ? 'Percorso completato. Ora verifica il risultato reale e chiudi la segnalazione solo se risolta.' : 'Passaggio confermato e checkpoint salvato.' })
    } catch (error) {
      setWorkspaceState((current) => ({ ...current, busy: false, error: workspaceErrorLabel(error), success: '' }))
      if ((error?.code || error?.message) === 'task_revision_conflict') {
        getIssueWorkspace({ hotelId, issueId: issue.id }).then((task) => setWorkspaceState((current) => ({ ...current, task }))).catch(() => {})
      }
    }
  }
  ...
  const buildSummary = async () => {
    ...
    try {
      ...
      onCompletionSummary?.(summary)
      setWorkspaceState((current) => ({ ...current, busy: false, success: 'Riepilogo preparato nelle note di completamento. Controllalo prima di chiudere.', error: '' }))
    } catch (error) {
      setWorkspaceState((current) => ({ ...current, busy: false, error: workspaceErrorLabel(error), success: '' }))
    }
  }
    ...
    <section className="rs-randai-suggestion rs-randai-workspace" aria-label="RandAI per questa segnalazione" data-testid="randai-issue-suggestion">
      <div className="rs-randai-suggestion__head">
        ...
        <div className="rs-randai-workspace__title">
          ...
          <small>{progress ? `${progress.label}${progress.next ? ` · prossimo: ${progress.next}` : ''}` : 'Assistente operativo della segnalazione'}</small>
        </div>
        <button type="button" className="rs-randai-workspace__toggle" onClick={() => setOpen((value) => !value)} aria-expanded={open} data-testid="randai-workspace-toggle">
          {open ? 'Chiudi' : progress ? 'Continua con RandAI' : 'Apri RandAI'}
        </button>
      </div>
      ...
      {open && (
        <div className="rs-randai-workspace__body" data-testid="randai-workspace-body">
          <div className="rs-randai-tabs" role="tablist" aria-label="Strumenti RandAI">
            {TABS.map(([key, label]) => <button type="button" key={key} role="tab" aria-selected={tab === key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}</button>)}
          </div>
          ...
          {tab === 'analyze' && (
            <div className="rs-randai-pane">
              {guidanceState.loading ? <p className="rs-randai-suggestion__muted">Analizzo contesto, impianto e dati disponibili…</p> : suggestion ? <>
                <p>{suggestion.text}</p>
                {suggestion.detail && <small className="rs-randai-suggestion__detail">{suggestion.detail}</small>}
                {suggestion.caution && <small className="rs-randai-suggestion__caution">{suggestion.caution}</small>}
                <small className="rs-randai-suggestion__source">Fonte: {suggestion.source}</small>
            ...
            <div className="rs-randai-pane">
              {workspaceState.loading ? <p className="rs-randai-suggestion__muted">Carico il percorso salvato…</p> : workspaceState.task ? <>
                ...
                {progress?.next ? <>
                  ...
                  {canComplete && issue.status !== 'done' && <button type="button" className="rs-randai-actions__primary" disabled={workspaceState.busy} onClick={buildSummary}>Prepara riepilogo intervento</button>}
                </>}
              </> : guidance?.procedure ? <>
                <p>È disponibile una procedura interna verificata. RandAI può trasformarla in un percorso persistente con checkpoint, così puoi uscire e riprendere da qui.</p>
                <button type="button" className="rs-randai-actions__primary" disabled={workspaceState.busy} onClick={startGuide}>{workspaceState.busy ? 'Avvio…' : 'Avvia percorso guidato'}</button>
              </> : <p className="rs-randai-suggestion__muted">Non esiste ancora una procedura verificata adatta. Usa Analizza e Casi simili; RandAI non genera una procedura operativa non approvata.</p>}
            </div>
          )}
          ...
          {tab === 'procedure' && (
            <div className="rs-randai-pane">
              {guidance?.procedure ? <>
                <strong>{guidance.procedure.title}</strong>
                {guidance.procedure.summary && <p>{guidance.procedure.summary}</p>}
                <ol className="rs-randai-procedure">{(guidance.procedure.steps || []).map((step, index) => <li key={`${index}-${step}`}>{step}</li>)}</ol>
                {guidance.procedure.caution && <small className="rs-randai-suggestion__caution">{guidance.procedure.caution}</small>}
                <small className="rs-randai-suggestion__source">Fonte: {guidance.procedure.sourceLabel || 'Procedura interna approvata'}</small>
              </> : <p className="rs-randai-suggestion__muted">Nessuna procedura interna approvata collegata a questa segnalazione.</p>}
            </div>
          )}
          ...
          {hasActions && !actionState.plan && (
            <details className="rs-randai-actions" data-testid="randai-action-gateway">
              <summary>Azioni sicure su RandApp</summary>
              <small>Ogni modifica passa dal Gateway: permessi, struttura, rischio, conferma, esecuzione e verifica server-side.</small>
              {canEdit && <div className="rs-randai-actions__row" aria-label="Cambia urgenza">{['alta', 'media', 'bassa'].filter((priority) => priority !== issue.urgency).map((priority) => <button type="button" key={priority} disabled={actionState.busy} onClick={() => prepare('issue.update_priority', { priority })}>Urgenza {priority}</button>)}</div>}
