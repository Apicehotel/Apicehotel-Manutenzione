...
const DEFAULT_HIDDEN = ['quick', 'recent', 'weather']
const defaultLayout = () => ({ order: [...DEFAULT_WIDGET_ORDER], hidden: [...DEFAULT_HIDDEN] })

const storageKey = (user, hotel) => `randapp.home.widgets.v2:${user?.id || user?.name || 'user'}:${hotel?.id || 'hotel'}`
...
export default function Home({ user, hotel, onNavigate, personalizeSignal = 0 }) {
  ...
  }), [editing, onNavigate, openIssues.length, openUrgents.length, recent, todayInterventions.length, user, weatherVisual])
  ...
  return (
    <div data-testid="home-view" className={editing ? 'rs-home is-editing' : 'rs-home'}>
      <style>{`
        ...
        @media (max-width:720px){.rs-home-grid{gap:8px}.rs-home-widget>.rs-home-widget__card{min-height:126px;padding-left:10px;padding-right:10px}.rs-home-grid[style*="--home-cols:3"] .rs-stat__icon{transform:scale(.82);transform-origin:left top}.rs-home-grid[style*="--home-cols:3"] .rs-stat__count{font-size:1.65rem}.rs-home-grid[style*="--home-cols:3"] .rs-stat h3{font-size:.8rem}.rs-home-grid[style*="--home-cols:3"] .rs-stat p{font-size:.7rem}.rs-weather-tile__icon{font-size:1.8rem}}
        @media (max-width:390px){.rs-home-grid{gap:6px}.rs-home-widget>.rs-home-widget__card{min-height:116px}.rs-home-grid[style*="--home-cols:3"] .rs-stat__icon{display:none}.rs-home-grid[style*="--home-cols:3"] .rs-stat__count{font-size:1.45rem}.rs-home-grid[style*="--home-cols:3"] .rs-stat h3{font-size:.73rem}.rs-home-grid[style*="--home-cols:3"] .rs-stat p{display:none}.rs-weather-tile strong{font-size:.8rem}.rs-weather-tile span:last-child{font-size:.66rem}}
      `}</style>
