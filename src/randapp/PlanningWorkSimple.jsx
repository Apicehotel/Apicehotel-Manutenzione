...
function NewWorkSheet({ open, onClose, weekStart, hotel, user, onSaved }) {
  ...
  return <Sheet open={open} onClose={onClose} className="rs-insert-shell">
    <form onSubmit={submit} style={{display:'grid',gap:18}}>
      ...
      <div style={{display:'grid',gap:9}}><strong>Settimana</strong><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,padding:4,border:'1px solid var(--rs-line)',borderRadius:14,background:'var(--rs-surface-2)'}}><button type="button" className={`rs-btn ${weekOffset===0?'rs-btn--primary':'rs-btn--ghost'}`} onClick={()=>setWeekOffset(0)}>Questa · {rangeLabel(weekStart)}</button><button type="button" className={`rs-btn ${weekOffset===1?'rs-btn--primary':'rs-btn--ghost'}`} onClick={()=>setWeekOffset(1)}>Seguente · {rangeLabel(addDays(weekStart,7))}</button></div></div>
...
function WorkRow({ item, user, onChanged }) {
  ...
  return <article style={{border:'1px solid var(--rs-line)',background:'var(--rs-surface)',borderRadius:16,padding:12,display:'grid',gap:8}}>
    <div style={{display:'grid',gridTemplateColumns:'44px minmax(0,1fr) auto',gap:10,alignItems:'center'}}><button type="button" onClick={()=>act(done?'pending':'done')} aria-label={done?'Riapri lavoro':'Segna fatto'} style={{width:44,height:44,borderRadius:12,border:'none',background:done?'var(--rs-ok)':finish?'var(--rs-warn)':'var(--rs-surface-2)',color:done?'white':'var(--rs-text)',fontSize:20}}>{done?'✓':'·'}</button><div style={{minWidth:0}}><strong style={{display:'block',textDecoration:done?'line-through':'none'}}>{item.description}</strong>{done&&item.doneBy&&<small style={{display:'block',marginTop:4,color:'var(--rs-ok)'}}>Fatto da {item.doneBy}{item.doneAt?` · ${new Date(item.doneAt).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'})}`:''}</small>}{finish&&<small style={{display:'block',marginTop:4,color:'var(--rs-warn)',fontWeight:800}}>Da finire</small>}</div><button type="button" className="rs-btn rs-btn--ghost" onClick={async()=>{await deletePlanningWorkDay(item.id);onChanged?.()}} aria-label="Elimina"><Icon name="trash"/></button></div>
