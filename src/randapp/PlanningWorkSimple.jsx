...
function NewWorkSheet({ open, onClose, weekStart, hotel, user, onSaved }) {
  ...
  useEffect(()=>{ if(open){ setDescription(''); setSelected([]) } },[open])
...
export default function PlanningWorkSimple({ hotel, user, openRequest=0 }) {
  ...
  const weekLabel=`${weekStart.getDate()}/${weekStart.getMonth()+1} – ${addDays(weekStart,6).getDate()}/${addDays(weekStart,6).getMonth()+1}`
