...
const SERIAL_STATUSES = ['available', 'in_use', 'maintenance', 'retired', 'lost']
const SERIAL_CONDITIONS = ['ok', 'attention', 'damaged']

function ScannerSheet({ open, hotel, onClose, onFound }) {
...
function StocktakeSheet({ open, hotel, items, locations, onClose, onReload }) {
  ...
  const cancel = async () => { setBusy(true); try { await cancelStocktake(take.id); await load() } catch (e) { setError(e.message) } finally { setBusy(false) } }
  ...
  return <Sheet open={open} onClose={onClose} title="Inventario fisico" className="rs-inventory-sheet"><div className="rs-actions-stack">
    {!take ? <><Field label="Ambito"><select className="rs-select" value={locationId} onChange={(e) => setLocationId(e.target.value)}><option value="">Intera struttura</option>{locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select></Field><Button variant="primary" disabled={busy} onClick={start}>Apri inventario</Button><p className="rs-field__hint">All'apertura viene congelata la giacenza attesa. La chiusura applica tutte le differenze in una sola transazione.</p></> : <>
      ...
      <div className="rs-form-actions"><Button variant="ghost" disabled={busy} onClick={cancel}>Annulla inventario</Button><Button variant="primary" disabled={!complete || busy} onClick={finish}>Chiudi e riconcilia</Button></div>
    </>}
    {error && <p className="rs-error">{error}</p>}
  </div></Sheet>
}
...
function TransferSheet({ open, hotel, items, onClose, onReload }) {
  ...
  const cancel = async (id) => { setBusy(true); try { await cancelInventoryTransfer(id); await load(); await onReload() } catch (e) { setError(e.message) } finally { setBusy(false) } }
  ...
  return <Sheet open={open} onClose={onClose} title="Trasferimenti" className="rs-inventory-sheet"><div className="rs-actions-stack">
    ...
    <h3>In transito / recenti</h3>{transfers.length === 0 ? <p className="rs-field__hint">Nessun trasferimento.</p> : transfers.map((t) => { const name = t.itemSnapshot?.name || items.find((i) => i.id === t.sourceItemId)?.name || 'Articolo trasferito'; return <Card key={t.id}><div className="rs-inventory-transfer-row"><div><strong>{name}</strong><small>{t.sourceHotelId} → {t.destinationHotelId} · {fmt(t.quantity)} {t.itemSnapshot?.unit || ''}</small></div><Badge>{t.status === 'received' ? 'Ricevuto' : t.status === 'cancelled' ? 'Annullato' : 'In transito'}</Badge>{t.status === 'in_transit' && t.destinationHotelId === hotel.id && <Button variant="primary" disabled={busy} onClick={() => receive(t.id)}>Ricevi</Button>}{t.status === 'in_transit' && t.sourceHotelId === hotel.id && <Button variant="ghost" disabled={busy} onClick={() => cancel(t.id)}>Annulla</Button>}</div></Card> })}
    {error && <p className="rs-error">{error}</p>}
  </div></Sheet>
}
...
function TraceSheet({ open, hotel, items, locations, onClose }) {
  ...
  const changeSerial = async (unit, changes) => { try { await updateSerialUnit(unit.id, hotel.id, changes); await load() } catch (e) { setError(e.message) } }
  ...
  return <Sheet open={open} onClose={onClose} title="Seriali e compatibilità" className="rs-inventory-sheet"><div className="rs-actions-stack">
    ...
    {serials.map((s) => <Card key={s.id}><div className="rs-inventory-serial-row"><div><strong>{s.assetTag}</strong><small>{s.serialNumber}{s.barcode ? ` · ${s.barcode}` : ''}</small></div><select className="rs-select" value={s.status} onChange={(e) => changeSerial(s, { status: e.target.value })}>{SERIAL_STATUSES.map((v) => <option key={v} value={v}>{v}</option>)}</select><select className="rs-select" value={s.condition} onChange={(e) => changeSerial(s, { condition: e.target.value })}>{SERIAL_CONDITIONS.map((v) => <option key={v} value={v}>{v}</option>)}</select></div></Card>)}
...
export default function InventoryBlock2Panel({ user, hotel, items, locations, onSelectItem, onReload }) {
  ...
  useEffect(() => { const incoming = new URLSearchParams(window.location.search).get('inventoryCode'); if (incoming) setSheet('scan') }, [hotel.id])
  const actions = useMemo(() => [
    ['scan','search','Scansiona','QR, barcode, seriale'],['label','package','Etichetta QR','Genera e stampa'],['stocktake','check','Inventario','Conta e riconcilia'],['trace','gear','Seriali','Tracciabilità e compatibilità'],['transfer','arrowRight','Trasferisci','Tra strutture'],
  ], [])
  return <><div className="rs-inventory-block2-actions">{actions.map(([key, icon, title, subtitle]) => <Card as="button" key={key} onClick={() => setSheet(key)} disabled={!canEdit && key !== 'scan' && key !== 'label'}><Icon name={icon} /><span><strong>{title}</strong><small>{subtitle}</small></span></Card>)}</div>
    <ScannerSheet open={sheet === 'scan'} hotel={hotel} onClose={() => setSheet('')} onFound={onSelectItem} />
