...
export default function RandChatAI({ open, groupId, groupName, onClose }) {
  ...
  return <div className="randchat-modal-backdrop" onClick={onClose}>
    <section className="randchat-modal randchat-modal--ai" onClick={(e) => e.stopPropagation()}>
      <header><div><h2>RandAI · {groupName || 'Gruppo'}</h2><small>Legge solo il contesto operativo autorizzato di questo gruppo. Mai i DM.</small></div><button className="randchat-iconbtn" onClick={onClose}>×</button></header>
      <form className="randchat-ai-form" onSubmit={ask}>
      ...
      {result && !guidance && <p className="randchat-muted">Nessuna procedura o conoscenza approvata pertinente trovata.</p>}
      {guidance && <div className="randchat-result">
        {procedure && <article className="randchat-procedure"><b>📘 {procedure.title}</b><p>{procedure.summary}</p>{procedure.caution && <small>⚠️ {procedure.caution}</small>}</article>}
        ...
        <small className="randchat-muted">Fonte: {guidance.source || 'conoscenza interna approvata'}</small>
      </div>}
    </section>
  </div>
}
