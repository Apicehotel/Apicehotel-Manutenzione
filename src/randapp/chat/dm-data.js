...
export async function fetchDmMessages(threadId, userId, limit = 120) {
  ...
  const { data, error } = await client.rpc('chat_dm_list_messages', {
...
export async function sendDmMessage({ threadId, userId, body }) {
  const client = ensureClient()
  ...
  const { data, error } = await client.rpc('chat_dm_send_message', {
    p_thread_id: threadId,
