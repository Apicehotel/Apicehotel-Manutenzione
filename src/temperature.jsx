...
export function TemperatureSensors({ hotel }) {
  ...
  const load = useCallback(async () => {
    ...
    setSensors(data || [])
    setError(requestError ? 'Impossibile caricare i sensori.' : '')
    setLoading(false)
  }, [hotel.id])
  ...
  return <section className="temperature-page plant-page" aria-labelledby="temperature-title">
    <header className="temperature-heading">
      <div>
        <h1 id="temperature-title">Sensori</h1>
        <p>{hotel.name} · dati eWeLink</p>
      </div>
    ...
    {loading && !sensors.length ? <div className="temperature-empty">Carico i sensori…</div> : !sensors.length ? <div className="temperature-empty">Nessun sensore disponibile.</div> : <>
      {switchSections.map((section) => (
        <section className="plant-section" key={section.id} aria-labelledby={`sensor-section-${section.id}`}>
          <div className="plant-section__head"><h2 id={`sensor-section-${section.id}`}>{section.label}</h2></div>
                ...
                {group.sensors.map((sensor) => {
                  ...
                  return <article className="plant-state-card" key={sensor.device_id}>
                    <div>
                      ...
                      <small>{sensor.online ? 'Online' : '⚠️ Offline'} · agg. {formatUpdatedAt(sensor.aggiornato_il)}</small>
                    </div>
      ...
      {!!temperatures.length && <section className="plant-section" aria-labelledby="sensor-section-temperature">
        <div className="plant-section__head"><h2 id="sensor-section-temperature">Temperature</h2></div>
          ...
          {temperatures.map((sensor) => {
            ...
            return <article className={`temperature-card ${sensor.in_allerta ? 'in-alert' : ''}`} key={sensor.device_id}>
              <div>
                ...
                <small>{sensor.online ? 'Online' : '⚠️ Offline'} · agg. {formatUpdatedAt(sensor.aggiornato_il)}{sensor.in_allerta ? ' · sopra i 20 °C' : ''}</small>
              </div>
