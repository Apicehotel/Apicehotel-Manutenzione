...
export async function findInventoryByCode(hotelId, rawCode) {
  ...
  const { data: units, error: unitError } = await supabase.from('inventory_serial_units').select('id,hotel_id,item_id,serial_number,asset_tag,barcode').eq('hotel_id', hotelId).eq('active', true).or(`serial_number.eq.${code},asset_tag.eq.${code},barcode.eq.${code}`).limit(1)
  if (unitError) throw unitError
  if (units?.[0]) return { kind: 'serial', itemId: units[0].item_id, serial: units[0] }
  const { data: locations, error: locationError } = await supabase.from('inventory_locations').select('id,hotel_id,name,scan_code').eq('hotel_id', hotelId).eq('active', true).eq('scan_code', code).limit(1)
  if (locationError) throw locationError
  if (locations?.[0]) return { kind: 'location', location: locations[0] }
  return null
}
