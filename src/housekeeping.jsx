...
export function Housekeeping({ user }) {
  ...
  return <section className="housekeeping-page">
    ...
    <div className="hk-structures">{STRUCTURES.map((item)=><button className={structure===item?'active':''} onClick={()=>setStructure(item)} key={item}>{item}</button>)}</div>
    <div className="hk-floors">{FLOORS.map((item)=>{const count=day.filter((room)=>room.struttura===structure&&Number(room.piano)===item&&room.stato_slope!=='libera'&&!['fatto','nondist'].includes(workByRoom[room.camera]?.stato)).length;return <button className={floor===item?'active':''} onClick={()=>setFloor(item)} key={item}><strong>{item}°</strong><small>{count} da fare</small></button>})}</div>
    ...
    {loading?<div className="temperature-empty">Carico Housekeeping…</div>:!rooms.length?<div className="temperature-empty">Carica il file Slope di oggi per iniziare.</div>:<div className="hk-grid">{rooms.map((room)=><article className={`hk-room ${room.stato_slope} ${room.lavoro}`} key={room.camera} onClick={()=>setOpen(room)}><div><strong>{room.camera}</strong><span>{room.tipologia||'Camera'}</span></div><b>{slopeLabels[room.stato_slope]}</b><small>{workLabels[room.lavoro]}</small>{room.note&&<em>Nota</em>}</article>)}</div>}
...
function RoomSheet({room,canEdit,onClose,onState,onSave}){
  ...
  return <div className="hk-backdrop" onClick={onClose}><section className="hk-sheet" role="dialog" aria-modal="true" onClick={(e)=>e.stopPropagation()}><header><div><small>Camera</small><h2>{room.camera}</h2></div><button onClick={onClose}>✕</button></header><div className="hk-details"><p><strong>{slopeLabels[room.stato_slope]}</strong>{room.arrivo&&` · arrivo ${room.arrivo}`}{room.partenza&&` · partenza ${room.partenza}`}</p><p>{room.tipologia||'Tipologia non indicata'}{room.letti&&` · ${room.letti}`}</p>{room.note&&<p>Note: {room.note}</p>}</div>{canEdit&&<><div className="hk-actions">{Object.entries(workLabels).map(([key,label])=><button className={room.lavoro===key?'active':''} key={key} onClick={()=>onState(key)}>{label}</button>)}</div><form onSubmit={(e)=>{e.preventDefault();onSave(fields)}}><label>Stato soggiorno<select value={fields.stato_slope} onChange={(e)=>setFields({...fields,stato_slope:e.target.value})}>{Object.entries(slopeLabels).map(([key,label])=><option value={key} key={key}>{label}</option>)}</select></label><label>Configurazione letti<input value={fields.letti} onChange={(e)=>setFields({...fields,letti:e.target.value})}/></label><label>Note<textarea rows="3" value={fields.note} onChange={(e)=>setFields({...fields,note:e.target.value})}/></label><button className="primary">Salva modifica</button></form></>}</section></div>
}
