...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let urgentId = String(body?.urgent_id || "").trim();
    ...
    let severity = String(body?.severity || "urgente").trim() === "emergenza" ? "emergenza" : "urgente";

    if (urgent && urgentId) {
      const { data: urgentRow } = await admin.from("richieste_urgenti").select("id,hotel_id,nota,gravita,posizione,stato").eq("id", urgentId).eq("hotel_id", hotel).maybeSingle();
      if (!urgentRow) return json({ ok: false, error: "urgent_not_found" }, 404);
      note = String(urgentRow.nota || note).trim(); location = String(urgentRow.posizione || location).trim(); severity = urgentRow.gravita === "emergenza" ? "emergenza" : "urgente";
      const since = new Date(Date.now() - 120000).toISOString();
      const { data: duplicate } = await admin.from("notification_outbox").select("id").eq("channel", "push").eq("hotel_id", hotel).contains("metadata", { event_type: "urgent", urgent_id: urgentId }).gte("created_at", since).limit(1).maybeSingle();
    ...
    if (!urgent) {
      ...
      const { data: issue } = await admin.from("segnalazioni").select("id,hotel_id,camera,categoria,note").eq("id", issueId).eq("hotel_id", hotel).maybeSingle();
      if (!issue) return json({ ok: false, error: "issue_not_found" }, 404);
      room = String(issue.camera || room).trim(); category = String(issue.categoria || category).trim(); note = String(issue.note || note).trim();
      const since = new Date(Date.now() - 120000).toISOString();
      const { data: duplicate } = await admin.from("notification_outbox").select("id").eq("channel", "push").eq("hotel_id", hotel).contains("metadata", { event_type: "issue_created", issue_id: issueId }).gte("created_at", since).limit(1).maybeSingle();
      if (duplicate) return json({ ok: true, enabled: true, status: "deduplicated", sent: 0, duplicate: true });
    }
    const defaultTitle = urgent ? `${severity === "emergenza" ? "EMERGENZA" : "URGENTE"} · ${hotelName}` : room ? `${hotelName} · ${room}` : `${hotelName} · Nuova segnalazione`;
    const defaultBody = urgent ? ([location,note].filter(Boolean).join(" · ") || "Aggiornamento urgente") : [category, note].filter(Boolean).join(" · ") || "Nuova segnalazione di manutenzione";
    const title = String(body?.title || defaultTitle).slice(0, 120);
    const messageBody = String(urgent ? defaultBody : defaultBody).slice(0, 500);
    const tag = urgent ? `avviso-urgente-${urgentId || hotel}` : `segnalazione-${issueId}`;
    const targetUrl = urgent ? `/?notification=urgent&hotel_id=${encodeURIComponent(hotel)}${urgentId ? `&urgent_id=${encodeURIComponent(urgentId)}` : ""}` : `/?notification=issue_created&hotel_id=${encodeURIComponent(hotel)}&issue_id=${encodeURIComponent(issueId)}`;
    const metadata = { requested_by: userData.user.id, event_type: eventType, issue_id: issueId || null, urgent_id: urgentId || null, severity: urgent ? severity : null };
    ...
    const vapidPublic = secretMap.get("vapid_public"), vapidPrivate = secretMap.get("vapid_private"), vapidSubject = secretMap.get("vapid_subject") || "mailto:appmanutenzioneapice@gmail.com";
    if (!vapidPublic || !vapidPrivate) { if (outboxRow) await admin.from("notification_outbox").update({ status: "failed", error: "vapid_keys_missing" }).eq("id", outboxRow.id); return json({ ok: false, error: "vapid_keys_missing" }, 500); }
    webpush.setVapidDetails(vapidSubject, vapidPublic, vapidPrivate);
    const payload = JSON.stringify({ title, body: messageBody, tag, url: targetUrl, urgent, eventType, issueId: issueId || null, urgentId: urgentId || null, hotelId: hotel, severity });
    let sent = 0; const expiredIds: string[] = [];
    await Promise.all(uniqueSubs.map(async (sub: any) => { try { await webpush.sendNotification({ endpoint: sub.endpoint, keys: { p256dh: sub.p256dh, auth: sub.auth } }, payload); sent += 1; } catch (error: any) { if (error?.statusCode === 404 || error?.statusCode === 410) expiredIds.push(sub.id); else console.error("send-push delivery", error?.statusCode || "unknown", error?.message || "delivery_failed"); } }));
  ...
  } catch (error) { console.error("send-push", error instanceof Error ? error.message : "unknown"); return json({ ok: false, error: "send_failed" }, 500); }
});
