...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const publishBody:Record<string,unknown>={topic,title,message,priority:assignments?4:5,tags:assignments?["wrench","bell"]:reminders?["bell","memo"]:housekeeping?["broom","hotel"]:(test?["white_check_mark","bell"]:["rotating_light","warning"])};if(body?.url)publishBody.click=String(body.url).slice(0,1000);
    const res=await fetch(server,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(publishBody)});if(!res.ok){const text=await res.text().catch(()=>"");console.error("ntfy-alert delivery",res.status,text.slice(0,500));return json({ok:false,error:"delivery_failed",status:res.status,detail:text.slice(0,160)},502)}const delivered=await res.json().catch(()=>({}));return json({ok:true,status:"sent",id:delivered?.id||null,time:delivered?.time||null,test,channel});
