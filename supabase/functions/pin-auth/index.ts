...
async function ensureIdentity(legacy:any){
  ...
  if(!authUserId)throw new Error("CREDENTIAL_REQUIRED");
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const {data:legacy,error:legacyError}=await admin.from("utenti").select("id,nome,ruolo,hotels,puo_admin,department,telefono,email,phone_country_code,phone_verified,email_verified,deve_cambiare_pin,active,is_system_protected,in_struttura,in_struttura_dal").eq("id",legacyUserId).eq("active",true).or('is_system_protected.eq.false,nome.eq.Randagio').contains("hotels",[hotelId]).maybeSingle();
    ...
    if(!existingProfile?.auth_user_id)return json({ok:false,error:"Utente o PIN non validi"},401);
    const {data:credential}=await admin.from("auth_pin_credentials").select("pin_hash,failed_attempts,locked_until").eq("auth_user_id",existingProfile.auth_user_id).maybeSingle();if(!credential?.pin_hash)return json({ok:false,error:"Utente o PIN non validi"},401);if(credential.locked_until&&new Date(credential.locked_until).getTime()>Date.now())return json({ok:false,error:"Troppi tentativi. Riprova più tardi."},429);const valid=await bcrypt.compare(pin,credential.pin_hash);if(!valid){const failures=Number(credential.failed_attempts||0)+1,patch:Record<string,unknown>={failed_attempts:failures};if(failures>=5){patch.failed_attempts=0;patch.locked_until=new Date(Date.now()+10*60*1000).toISOString()}await admin.from("auth_pin_credentials").update(patch).eq("auth_user_id",existingProfile.auth_user_id);return json({ok:false,error:"Utente o PIN non validi"},401)}
    const {authUserId,internalEmail}=await ensureIdentity(legacy);await admin.from("auth_pin_credentials").update({failed_attempts:0,locked_until:null}).eq("auth_user_id",authUserId);
  ...
  }catch(error){const m=error instanceof Error?error.message:"unknown";if(m==="INACTIVE")return json({ok:false,error:"Utente disattivato"},403);if(m==="PROTECTED")return json({ok:false,error:"Account Admin accessibile solo dal pannello Admin"},403);if(m==="CREDENTIAL_REQUIRED")return json({ok:false,error:"Utente o PIN non validi"},401);console.error("pin-auth",m);return json({ok:false,error:"Errore temporaneo di accesso"},500)}
});
