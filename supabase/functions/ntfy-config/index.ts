...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const channels=[urgentTopic?{id:"urgent",label:"Avvisi urgenti",topic:urgentTopic,alias:aliasFor(hotelId,"urgent",notificationCode),priority:5}:null,reminderTopic?{id:"reminders",label:`Promemoria · ${role}`,topic:reminderTopic,alias:aliasFor(hotelId,"reminders",notificationCode),priority:5}:null,assignmentTopic?{id:"assignments",label:"Interventi assegnati a te",topic:assignmentTopic,alias:aliasFor(hotelId,"assignments",notificationCode),priority:4}:null,housekeepingTopic?{id:"housekeeping",label:"Housekeeping",topic:housekeepingTopic,alias:aliasFor(hotelId,"housekeeping",notificationCode),priority:5}:null].filter(Boolean);
  if(!channels.length)return json({ok:true,enabled:false,role,notification_code:notificationCode||null,channels:[],apps:APPS});const primary=channels[0] as any;
  return json({ok:true,enabled:true,server:alertServer,role,notification_code:notificationCode||null,channels,apps:APPS,topic:primary.topic,channel:primary.id,reminder_topic:reminderTopic||null,urgent_topic:urgentTopic||null,assignment_topic:assignmentTopic||null,housekeeping_topic:housekeepingTopic||null});
