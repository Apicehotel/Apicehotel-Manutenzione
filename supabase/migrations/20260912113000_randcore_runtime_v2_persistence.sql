...
create index if not exists randcore_events_hotel_occurred_idx
  on public.randcore_events (hotel_id, occurred_at desc)
  where hotel_id is not null;
create index if not exists randcore_events_correlation_idx
  on public.randcore_events (correlation_id);
...
create index if not exists randcore_jobs_status_updated_idx
  on public.randcore_jobs (status, updated_at);
create index if not exists randcore_jobs_event_idx
  on public.randcore_jobs (event_id);
create index if not exists randcore_jobs_lease_idx
  on public.randcore_jobs (lease_expires_at)
...
create index if not exists randcore_workers_heartbeat_idx
  on public.randcore_workers (last_heartbeat_at);
...
create unique index if not exists randcore_dead_letters_job_reason_uidx
  on public.randcore_dead_letters (job_id, reason);
...
-- Atomic claim with a finite lease. Only service_role may execute it.
...
-- Recover jobs abandoned by crashed/redeployed workers. Exhausted jobs are
-- dead-lettered in the same transaction, so restart recovery is deterministic.
...
as $$
declare
  job public.randcore_jobs;
  recovered public.randcore_jobs;
begin
  for job in
    select *
      from public.randcore_jobs
     where status = 'RUNNING'
