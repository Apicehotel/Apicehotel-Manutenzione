...
using (public.has_hotel_role(hotel_id, array['Direttore Centro Congressi']));
...
begin
  if (select auth.uid()) is null or not public.has_hotel_role(p_hotel_id, array['Direttore Centro Congressi']) then
  ...
  if not exists(select 1 from public.prenotazioni_sale where id=p_booking_id and hotel_id=p_hotel_id) then
    raise exception 'sale_layout_booking_scope_denied';
  end if;
