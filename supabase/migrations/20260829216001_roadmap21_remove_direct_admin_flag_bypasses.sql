...
create policy app_config_admin_update on public.app_config for update to authenticated using (public.has_any_randapp_admin()) with check (public.has_any_randapp_admin());

drop policy if exists role_permissions_admin_insert on public.role_permissions;
create policy role_permissions_admin_insert on public.role_permissions for insert to authenticated with check (public.has_any_randapp_admin());
drop policy if exists role_permissions_admin_update on public.role_permissions;
create policy role_permissions_admin_update on public.role_permissions for update to authenticated using (public.has_any_randapp_admin()) with check (public.has_any_randapp_admin());
drop policy if exists role_permissions_admin_delete on public.role_permissions;
create policy role_permissions_admin_delete on public.role_permissions for delete to authenticated using (public.has_any_randapp_admin());
...
with check (exists(select 1 from public.hotel_memberships hm where hm.auth_user_id=auth.uid() and hm.active and hm.role<>'RandAI' and (hm.can_access_admin or hm.role='admin') and hm.hotel_id=any(utenti.hotels)));
...
using (exists(select 1 from public.hotel_memberships hm where hm.auth_user_id=auth.uid() and hm.active and hm.role<>'RandAI' and (hm.can_access_admin or hm.role='admin') and hm.hotel_id=any(utenti.hotels)))
with check (exists(select 1 from public.hotel_memberships hm where hm.auth_user_id=auth.uid() and hm.active and hm.role<>'RandAI' and (hm.can_access_admin or hm.role='admin') and hm.hotel_id=any(utenti.hotels)));
