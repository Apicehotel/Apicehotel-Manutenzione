-- Punto 9: operational health, incident aggregation and safe recovery tools.
...
begin
  ...
  select count(*) into v_cron_problem
  from cron.job
  where jobname in ('reminder-worker-1m','sync-sensori-temperatura-secure','urgent-reminder-worker-30s','weather-alert-worker-10m')
    and active is not true;

  if v_cron_problem>0 or v_urgent_overdue>0 or v_outbox_problem>0 or (v_weather_checked is not null and v_weather_checked < now()-interval '30 minutes') then
    v_status := 'problem';
  ...
  select jsonb_build_object(
    'generated_at', now(),
    'status', v_status,
    'weather', jsonb_build_object('status', case when v_weather_checked is null then 'unknown' when v_weather_checked < now()-interval '30 minutes' then 'problem' else 'ok' end, 'level', v_weather_level, 'last_checked_at', v_weather_checked),
    ...
    'urgent_jobs', jsonb_build_object('status', case when v_urgent_overdue>0 then 'problem' when v_urgent_failed>0 then 'warning' else 'ok' end, 'failed_24h', v_urgent_failed, 'overdue', v_urgent_overdue,
      'failed', coalesce((select jsonb_agg(jsonb_build_object('id',id,'error',last_error,'updated_at',coalesce(updated_at,created_at)) order by coalesce(updated_at,created_at) desc)
        from (select id,last_error,updated_at,created_at from public.urgent_reminder_jobs where hotel_id=p_hotel_id and status='failed' order by coalesce(updated_at,created_at) desc limit 10) x), '[]'::jsonb)),
    ...
    'diagnostics', jsonb_build_object('status', case when v_diag_fatal>0 then 'warning' when v_diag_errors>0 then 'warning' else 'ok' end, 'errors_24h', v_diag_errors, 'fatal_24h', v_diag_fatal),
    'push', jsonb_build_object('status', case when v_push_count>0 then 'ok' else 'warning' end, 'subscriptions', v_push_count),
    'cron', jsonb_build_object('status', case when v_cron_problem>0 then 'problem' else 'ok' end, 'inactive', v_cron_problem,
      'jobs', coalesce((select jsonb_agg(jsonb_build_object('name',jobname,'schedule',schedule,'active',active) order by jobname) from cron.job where jobname in ('reminder-worker-1m','sync-sensori-temperatura-secure','urgent-reminder-worker-30s','weather-alert-worker-10m')), '[]'::jsonb)),
...
as $$
  ...
  where d.hotel_id=p_hotel_id
    ...
    and public.can_admin_hotel(p_hotel_id)
...
create or replace function public.retry_failed_urgent_job(p_hotel_id text, p_job_id uuid)
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.urgent_reminder_jobs
   ...
   where id=p_job_id and hotel_id=p_hotel_id and status='failed';
  return found;
end;
$$;
revoke all on function public.retry_failed_urgent_job(text,uuid) from public, anon;
grant execute on function public.retry_failed_urgent_job(text,uuid) to authenticated;
