...
// App boundary: keep login/auth shell small and load authenticated UI only when needed.
const appPath = 'src/randapp/App.jsx'
let app = await readFile(appPath, 'utf8')
app = app.replace("import { useEffect, useMemo, useState } from 'react'", "import { lazy, Suspense, useEffect, useMemo, useState } from 'react'")
app = app.replace("import Shell from './Shell.jsx'\n", '').replace("import Settings from './Settings.jsx'\n", '')
if (!app.includes("const Shell = lazy(")) app = app.replace("const EVENT = 'apice-session-changed'", "const Shell = lazy(() => import('./Shell.jsx'))\nconst Settings = lazy(() => import('./Settings.jsx'))\n\nconst EVENT = 'apice-session-changed'")
app = app.replace("if (ok) return <Settings onExit={onExit || onBack} />", "if (ok) return <Suspense fallback={<Spinner label=\"Carico impostazioni…\" />}><Settings onExit={onExit || onBack} /></Suspense>")
app = app.replace("if (session) return <Shell session={session} onLogout={onLogout} onSwitchHotel={(id) => { saveSession({ ...session, hotelId: id }); setSession(loadSession()) }} />", "if (session) return <Suspense fallback={<Spinner label=\"Avvio RandApp…\" />}><Shell session={session} onLogout={onLogout} onSwitchHotel={(id) => { saveSession({ ...session, hotelId: id }); setSession(loadSession()) }} /></Suspense>")
await writeFile(appPath, app)

// Shell boundary: Home remains immediate once authenticated; secondary features are route-lazy.
...
// Avoid a single utility module pulling Housekeeping + Temperature into unrelated routes.
