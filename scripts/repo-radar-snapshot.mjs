...
import { buildRepoRadarSnapshot, RepoRadarUsageMode } from '../src/randai/discovery/repo-radar.js'
...
import { assertRepoRadarSourcePolicy, summarizeRepoRadarSourceCoverage } from '../src/randai/discovery/repo-radar-sources.js'

const token=process.env.GITHUB_TOKEN||''
const now=Date.now()
const staleMs=180*24*60*60*1000
const requestHeaders={'User-Agent':'Rand-Repo-Radar/4.0'}
...
const RANDUI_SECTORS=[
  ...
  {id:'DESIGN_SYSTEM',priority:1,query:'react design system component library tokens storybook accessibility figma'},
  ...
  {id:'VISUAL_BUILDER',priority:.92,query:'react visual page builder component registry drag drop editor figma'},
  {id:'VISUAL_TESTING',priority:1,query:'playwright visual regression responsive layout screenshot testing figma'},
...
const PROVIDERS=[
  ...
  {id:'GITEE',search:discoverGitee},
  {id:'NPM',search:discoverNpm},
  {id:'CRATES',search:discoverCrates},
  {id:'HUGGINGFACE',search:discoverHuggingFace},
  {id:'OPENVSX',search:discoverOpenVsx},
]

assertRepoRadarSourcePolicy()
...
function normalizeRepositoryUrl(raw){
  ...
  if(/^git@gitee\.com:/i.test(value)) value=value.replace(/^git@gitee\.com:/i,'https://gitee.com/')
...
function metaCandidate(meta,profile){
  ...
  const score=discoveryScore(meta,profile)
  return {
    ...
    usageMode:RepoRadarUsageMode.INTERNAL_EVALUATION,
    ...
    discovery:{profile:profile.id,category:profile.category,sector:profile.sector,inventoryRef:profile.inventoryRef||null,platform:meta.platform,score,description:meta.description||null},
...
async function discoverGitee(profile){
  const data=await requestJson(`https://gitee.com/api/v5/search/repositories?q=${encodeURIComponent(profile.query)}&per_page=4&sort=stars_count&order=desc`)
  return (Array.isArray(data)?data:[]).map((repo)=>({platform:'GITEE',fullName:repo.full_name||repo.path||repo.name,repository:repo.html_url||repo.url,description:repo.description,pushedAt:repo.updated_at||repo.pushed_at,stars:repo.stargazers_count||repo.stars_count,archived:repo.archived,license:repo.license||null,openIssues:repo.open_issues_count,forks:repo.forks_count,defaultBranch:repo.default_branch,language:repo.language,topics:repo.topics||[]})).filter((item)=>normalizeRepositoryUrl(item.repository))
}
...
async function discoverNpm(profile){
  ...
  for(const entry of data.objects||[]){
    ...
    if(repository) items.push({platform:'NPM',fullName:pkg.name||repository,repository,description:pkg.description,pushedAt:entry.updated,stars:0,archived:false,license:pkg.license||null,openIssues:0,forks:0,defaultBranch:null,language:'JavaScript/TypeScript',topics:pkg.keywords||[]})
  }
  return items
}

async function discoverCrates(profile){
  const data=await requestJson(`https://crates.io/api/v1/crates?q=${encodeURIComponent(profile.query)}&page=1&per_page=4&sort=downloads`)
  return (data.crates||[]).map((crate)=>({platform:'CRATES',fullName:crate.id||crate.name,repository:normalizeRepositoryUrl(crate.repository),description:crate.description,pushedAt:crate.updated_at||crate.created_at,stars:0,archived:false,license:crate.license||null,openIssues:0,forks:0,defaultBranch:null,language:'Rust',topics:crate.keywords||[]})).filter((item)=>item.repository)
}

async function discoverHuggingFace(profile){
  const data=await requestJson(`https://huggingface.co/api/spaces/semantic-search?q=${encodeURIComponent(profile.query)}`)
  const spaces=Array.isArray(data)?data:(data.spaces||data.results||[])
  return spaces.slice(0,4).map((space)=>({platform:'HUGGINGFACE',fullName:space.id||space.name,repository:`https://huggingface.co/spaces/${space.id||space.name}`,description:space.description||space.cardData?.description,pushedAt:space.lastModified||space.updatedAt,stars:space.likes||0,archived:false,license:space.cardData?.license||null,openIssues:0,forks:0,defaultBranch:null,language:space.sdk||'Space',topics:space.tags||[]})).filter((item)=>item.fullName&&item.fullName.includes('/'))
}

async function discoverOpenVsx(profile){
  const data=await requestJson(`https://open-vsx.org/api/-/search?query=${encodeURIComponent(profile.query)}&size=4`)
  return (data.extensions||[]).map((extension)=>({platform:'OPENVSX',fullName:`${extension.namespace}/${extension.name}`,repository:normalizeRepositoryUrl(extension.repository)||`https://open-vsx.org/extension/${extension.namespace}/${extension.name}`,description:extension.description,pushedAt:extension.timestamp||extension.lastUpdated,stars:extension.averageRating||0,archived:false,license:extension.license||null,openIssues:0,forks:0,defaultBranch:null,language:'IDE Extension',topics:extension.tags||[]}))
}
...
async function enrich(candidate){
  ...
  try{
    const data=await requestJson(`https://api.github.com/repos/${match[1]}/${match[2]}`,{headers:githubHeaders})
    const pushedAt=data.pushed_at||null
    return {...candidate,stars:Number(data.stargazers_count||0),archived:Boolean(data.archived),maintained:!data.archived&&Boolean(pushedAt)&&(now-Date.parse(pushedAt))<=staleMs,license:data.license?.spdx_id||candidate.license||null,evaluatedAt:new Date().toISOString(),repositoryMeta:{pushedAt,openIssues:Number(data.open_issues_count||0),forks:Number(data.forks_count||0),defaultBranch:data.default_branch||null,language:data.language||null,topics:data.topics||[]}}
  }catch(error){
...
function boundedSelection(found){
  ...
  const buckets=new Map(),selected=[],selectedUrls=new Set()
...
async function discover(){
  ...
  for(const profile of SEARCH_PROFILES){
    ...
    settled.forEach((result,index)=>{
      ...
      if(result.status==='rejected'){providerStats[provider.id].errors++;console.warn(`Repo Radar ${provider.id}/${profile.id} failed: ${result.reason?.message||result.reason}`);return}
