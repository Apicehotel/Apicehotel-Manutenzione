...
export function createAppJwt({
  ...
  if (!appId || !privateKey) {
    throw new Error('Configurazione GitHub App incompleta')
  }
...
export function isAuthorized(request, expectedSecret) {
  ...
  const provided = authorization.startsWith('Bearer ')
    ...
    : request.headers['x-github-bridge-key']
...
export async function getInstallationAccess() {
  const configuredRepo =
    process.env.GITHUB_REPO || TARGET_REPO

  if (configuredRepo !== TARGET_REPO) {
    throw new Error(
      'Repository GitHub non consentito'
    )
  }
  ...
  const installation = await github(
    '/repos/Apicehotel/Apicehotel-Manutenzione/installation',
    {
      token: appJwt
    }
  )
...
export async function testRepositoryRead() {
  ...
  const repository = await github(
    '/repos/Apicehotel/Apicehotel-Manutenzione',
    {
      token: access.token
    }
  )
